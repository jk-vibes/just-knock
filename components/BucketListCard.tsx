...
import { MapPin, Navigation, CheckCircle2, Circle, Trash2, Image as ImageIcon, Calendar, Sparkles, Route } from 'lucide-react';
...
export const BucketListCard: React.FC<BucketListCardProps> = ({ 
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    setIsSwiping(true); // Disable transition for direct 1:1 movement
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Lock to horizontal swipe if dominant
    if (Math.abs(diffX) > Math.abs(diffY)) {
        if (diffX < 0) { // Only swipe left
            setOffsetX(diffX);
            // If dragging significantly, mark as dragging to prevent click
            if (Math.abs(diffX) > 10) isDragging.current = true;
        }
    }
  };

  const handleTouchEnd = () => {
    setIsSwiping(false); // Enable transition for snap back
    
    // Threshold for delete (approx 80px)
    if (offsetX < -80) { 
        onDelete(item.id);
        // Keep it offset visually until unmount or state update creates new list
        setOffsetX(-500); 
    } else {
        setOffsetX(0);
    }
  ...
  return (
    <div className="relative group rounded-2xl overflow-hidden touch-pan-y select-none">
      ...
      <div 
        className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm border overflow-visible ${isSwiping ? '' : 'transition-transform duration-300 ease-out'} ${isCompact ? 'p-2' : 'p-3.5'} ${isNearby ? 'border-orange-400 ring-1 ring-orange-100 dark:ring-orange-900/30' : 'border-gray-100 dark:border-gray-700 hover:shadow-md hover:border-red-200 dark:hover:border-gray-600'}`}
      ...
      >
        ...
        {/* Item Type Indicator (Dream Sparkle only for non-location items) */}
        {!hasCoordinates && (
            <div className={`absolute top-0 right-0 z-20 pointer-events-none transition-opacity duration-300 ${isCompact ? 'p-1.5' : 'p-2'}`}>
        ...
        {/* Trip Planner Icon (Top Right - For location items) */}
        {hasCoordinates && onOpenPlanner && (
            <button
                ...
                className={`absolute top-2 right-2 z-20 p-1.5 rounded-full bg-white/90 dark:bg-gray-800/90 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/30 shadow-sm hover:scale-110 hover:bg-blue-50 dark:hover:bg-blue-900/50 transition-all ${isCompact ? 'right-1.5 top-1.5 p-1' : ''}`}
        ...
        <div className="flex justify-between items-start gap-3 relative z-10">
            ...
            <div className={`flex items-start gap-2 ${isCompact ? 'mb-0.5' : 'mb-1.5'}`}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                        <h3 className={`font-semibold text-lg text-gray-800 dark:text-white truncate transition-colors ${item.completed ? 'line-through text-gray-400 dark:text-gray-500' : ''}`}>
                        {item.title}
                        </h3>
            ...
            {!isCompact && (
                <p className={`text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-1.5 ml-7 transition-colors ${item.completed ? 'opacity-50' : ''}`}>
                    {item.description}
                </p>
            )}
